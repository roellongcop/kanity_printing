...
class Portfolio extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['name', 'description', 'portfolio_category_id'], 'required'],
    ...
    public function getPortfolioCategory($status = 1)
    {
        return $this->hasOne(PortfolioCategory::className(), ['id' => 'portfolio_category_id'])
            ->alias('pc')
            ->onCondition(['pc.status' => $status]);
    }

    public function getCategoryName()
    {
        if (($model = $this->portfolioCategory) != null) {
            return $model->name;
        }
    }
    ...
    {
        $uploadPath = Yii::$app->general->createFolder(['uploads', 'portfolio', $this->categoryName]); 
