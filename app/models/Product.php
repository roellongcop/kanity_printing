...
class Product extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['service_id', 'created_by', 'updated_by', 'status'], 'integer'],
            [['name', 'tag_line', 'description', 'service_id'], 'required'],
    ...
    public function getService($status = 1)
    {
        return $this->hasOne(Service::className(), ['id' => 'service_id'])
            ->alias('sv')
            ->onCondition(['sv.status' => $status]);
    }
    ...
    {
        $uploadPath = Yii::$app->general->createFolder(['uploads', 'products', $this->serviceName]); 
